#!/usr/bin/env bash
#
# Track changes in php/doc-en and create issues in doc-de
# for files that need translation updates.
#
# How it works:
#   - Fetches all commits on doc-en master from the last 7 days
#   - For each commit, checks if a matching issue already exists in doc-de
#   - If not, creates one listing the DE files to update
#
...
# Ensure the sync-en label exists (idempotent; ignored if already present).
gh label create sync-en --color "0e8a16" --description "Sync from upstream doc-en" 2>/dev/null || true
...
while read -r SHA; do
  ...
  # Skip if no DE-relevant files
  if [ -z "$UPDATE_LIST" ] && [ -z "$NEW_LIST" ]; then
    echo "  -> No DE-relevant files, skipping."
    continue
  fi
